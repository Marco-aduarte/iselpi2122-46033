...
test('Create and update a task', async () => {
    ...
    await tasks.updateTask('rambo', actual.id, actual.due, 'clean-shoes', 'Clean running shoes')
    const allAfter = await tasks.getAll('rambo')
    expect(allAfter.length).toBe(2)

    const actualAfter = await tasks.getTask('rambo', shoes.id)
    expect(actualAfter.title).toBe('clean-shoes')
    expect(actualAfter.description).toBe('Clean running shoes')
    expect(actualAfter.due).toBe(actual.due)
    expect(shoes.id).toBe(actualAfter.id)
})