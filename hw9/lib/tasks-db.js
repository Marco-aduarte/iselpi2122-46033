...
const { use } = require('./tasks-web-api')
...
/**
 * updates the task with ìd of user username with new data
 * @param {String} username 
 * @param {String} id 
 * @param {Number} due 
 * @param {String} title 
 ...
 */
function updateTask(username, id, due, title, description) {
    deleteTask(username, id)
        .then(() => {
            const file = `task-${username}-${id}-${title}.json`
            return fs
                .writeFile(dataPath + file, JSON.stringify({id, dueDate: new Date(due), title, description}))
        })   
}
